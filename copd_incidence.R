...
copd_deaths_scotland <- copd_deaths |>  group_by(sex, age_grp, cal_year) |>  
  count(.groups = "drop")  # calculate numerator
...
copd_deaths_scotland <- copd_deaths_scotland |> 
  mutate(numerator = replace_na(numerator, 0))
...
data_copd <- rbind(as_tibble(dbGetQuery(channel, statement= query_sql("ANALYSIS.SMR01_PI"))),
                     as_tibble(dbGetQuery(channel, statement= query_sql("ANALYSIS.SMR01_HISTORIC"))) ) |> 
  janitor::clean_names() # variables to lower case
...
# Joining data with population (denominator)
deaths_admissions_scotland <- left_join(deaths_admissions_scotland, scottish_population, 
                                c("year", "age_grp", "sex")) |>  
...
data_agegroups <- deaths_admissions_scotland |>
 # mutate(sex = as.numeric(sex)) |>
  group_by(year, age_grp2, sex) |>
  select(-age_grp) |>
...
dz11_pop <- readRDS("/conf/linkage/output/lookups/Unicode/Populations/Estimates/DataZone2011_pop_est_2011_2022.rds") |> 
  janitor::clean_names() |> 
...
copd_dep <- data_copd |> 
  ...
  filter(year == 2023) |> # only data for latest year available 
  mutate(year = 2022)
