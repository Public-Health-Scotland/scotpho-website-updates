...
data_folder <- "/PHI_conf/ScotPHO/Website/Topics/Epilepsy/202112_update/"

# file path for shiny output - update analyst's folder
shiny_folder <- "/PHI_conf/ScotPHO/1.Analysts_space/Peter/epilepsy-shiny-chart/shiny_app/data/"
...
epilepsy_deaths <- tbl_df(dbGetQuery(channel, statement=
      ...
      WHERE date_of_registration between '1 January 1974' and '31 December 2020'
      AND sex <> 9
...
query_sql <- function(table) {
  paste0("SELECT distinct link_no linkNo, cis_marker CIS, max(age_in_years) age, min(ADMISSION_DATE) doadm,
         ...
         FROM ", table,
         " WHERE admission_date between '1 April 1994' and '31 March 2021' 
