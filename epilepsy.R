...
data_epilepsy <- tbl_df(dbGetQuery(channel, statement=
      "SELECT distinct link_no linkNo, cis_marker CIS, max(age_in_years) age, min(DR_POSTCODE) pc7,
              ...
              WHERE discharge_date between '1 April 2004' and '31 March 2019' 
...
data_epilepsy <- data_epilepsy %>% mutate(age_grp = case_when( 
  age < 5 ~ 1, age > 4 & age <10 ~ 2, age > 9 & age <15 ~ 3, age > 14 & age <20 ~ 4,
  age > 19 & age <25 ~ 5, age > 24 & age <30 ~ 6, age > 29 & age <35 ~ 7, 
  age > 34 & age <40 ~ 8, age > 39 & age <45 ~ 9, age > 44 & age <50 ~ 10,
  age > 49 & age <55 ~ 11, age > 54 & age <60 ~ 12, age > 59 & age <65 ~ 13, 
...
data_epilepsy_scotland <- data_epilepsy %>% group_by(age_grp, age_grp2, sex, year) %>% 
  count() %>% ungroup() # calculate numerator

# Joining data with population (denominator)
data_epilepsy_scotland <- full_join(data_epilepsy_scotland, scottish_population, 
                                  c("year", "age_grp", "sex")) %>% 
  rename(numerator = n, denominator = pop) # numerator and denominator used for calculation

data_epilepsy_scotland <- data_epilepsy_scotland %>% 
...
data_epilepsy_scotland$numerator[is.na(data_epilepsy_scotland$numerator)] <- 0 

# this section is for splitting ages under 10 and 10+ for males and females
data_agegroups <- data_epilepsy_scotland %>% group_by(year, age_grp2, sex) %>% 
  select(-age_grp) %>% 
...
all_epilepsy_chart <- create_chart_data(dataset = data_epilepsy_scotland, epop_total = 200000, filename = "epilepsy_scotland_all_chart")
